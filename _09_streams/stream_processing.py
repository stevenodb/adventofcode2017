...
import re
import sys
from typing import AnyStr, Callable, Dict, Iterator, Tuple

GARBAGE_HANDLERS: Dict[str, Callable[[Iterator, int], Tuple[str, int]]] = \
    {
        '!': lambda it, count: (next(it, None) if next(it, None) else None, count),
        '>': lambda it, count: (None, count),
        '.': lambda it, count: (next(it, None), count + 1)
    }


def garbage(it: Iterator, n_garbage: int) -> Tuple[str, int]:
    head = next(it, None)
    while head:
        for (pattern, handler) in GARBAGE_HANDLERS.items():
            if re.match(pattern, head):
                (head, n_garbage) = handler(it, n_garbage)
                break
    return next(it, None), n_garbage


HANDLERS: Dict[str, Callable[[int, int, Iterator], Tuple]] = \
    {
        '!': lambda score, level, n_garbage, it: (score, level, next(it, None) if next(it, None) else None, n_garbage),
        '<': lambda score, level, n_garbage, it: (score, level, *garbage(it, n_garbage)),
        '{': lambda score, level, n_garbage, it: (score + level, level + 1, next(it, None), n_garbage),
        '}': lambda score, level, n_garbage, it: (score, level - 1, next(it, None), n_garbage),
        ',': lambda score, level, n_garbage, it: (score, level, next(it, None), n_garbage),
    }


def process(stream: AnyStr, level: int = 1) -> Dict[str, int]:
    score = 0
    n_garbage = 0
    it = iter(stream)
    ...
    while head:
        for (pattern, handler) in HANDLERS.items():
            if re.match(pattern, head):
                (score, level, head, n_garbage) = handler(score, level, n_garbage, it)
                break
    return {'score': score, 'n_garbage': n_garbage}
