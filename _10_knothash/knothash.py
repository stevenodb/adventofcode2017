...
position. Move the current position forward by that length plus the skip size. Increase the skip
size by one. The list is circular; if the current position and the length try to reverse elements
beyond the end of the list, the operation reverses using as many extra elements as it needs from the
front of the list. If the current position moves past the end of the list, it wraps around to the
...
The list begins as [0] 1 2 3 4 (where square brackets indicate the current position). The first
length, 3, selects ([0] 1 2) 3 4 (where parentheses indicate the sublist to be reversed). After
reversing that section (0 1 2 into 2 1 0), we get ([2] 1 0) 3 4. Then, the current position moves
forward by the length, 3, plus the skip size, 0: 2 1 0 [3] 4. Finally, the skip size increases to 1.
The second length, 4, selects a section which wraps: 2 1) 0 ([3] 4. The sublist 3 4 2 1 is reversed
to form 1 2 4 3: 4 3) 0 ([1] 2. The current position moves forward by the length plus the skip size,
a total of 5, causing it not to move because it wraps around: 4 3 0 [1] 2. The skip size increases
to 2. The third length, 1, selects a sublist of a single element, and so reversing it has no effect.
The current position moves forward by the length (1) plus the skip size (2): 4 [3] 0 1 2. The skip
size increases to 3. The fourth length, 5, selects every element starting with the second: 4) ([3] 0
1 2. Reversing this sublist (3 0 1 2 4 into 4 2 1 0 3) produces: 3) ([4] 2 1 0. Finally, the current
position moves forward by 8: 3 4 2 1 [0]. The skip size increases to 4. In this example, the first
two numbers in the list end up being 3 and 4; to check the process, you can multiply them together
to produce 12.
...
def knot_hash(lengths, p_cur: int = 0, skip_size: int = 0, marks=None) -> List[int]:
    if marks is None:
        marks = [x for x in range(256)]

    for l in lengths:
        p_l = p_cur + l
        reverse_circular(marks, p_cur, p_l)
        p_cur = (p_l + skip_size) % len(marks)
        skip_size += 1

    return marks, p_cur, skip_size


def reverse_circular(lst: List, start: int, end: int):
    """
    Reverses items in lst from start position (inclusive) to end position (exclusive).
    If end < start, reverse will loop around the end of lst.
    """
    n = end - start if end >= start else len(lst) - start + end
...
65 ^ 27 ^ 9 ^ 1 ^ 4 ^ 3 ^ 40 ^ 50 ^ 91 ^ 7 ^ 6 ^ 0 ^ 2 ^ 5 ^ 68 ^ 22 = 64 Perform this operation on
each of the sixteen blocks of sixteen numbers in your sparse hash to determine the sixteen numbers
in your dense hash.
...
33efeb34ea91902bb2f59c9920caa6cd. 1,2,3 becomes 3efbe78a8d82f29979031a4aa0b16a9d. 1,2,4 becomes
63960835bcdc130f0b66d7ff4f6a5a8e. Treating your puzzle input as a string of ASCII characters, what
is the Knot Hash of your puzzle input? Ignore any leading or trailing whitespace you might
encounter.
"""


def to_dense_hash(lst):
    assert len(lst) == 256
    dense_hash = []
    for block in range(16):
        acc = lst[block * 16]
        for number in range(1, 16):
            acc = acc ^ lst[(block * 16) + number]
        dense_hash += [acc]
    return dense_hash


def sparse_hash(lengths):
    padded_lengths = lengths + [17, 31, 73, 47, 23]
    position = 0
    skip_size = 0
    marks = None
    for i in range(64):
        (marks, position, skip_size) = knot_hash(padded_lengths, position, skip_size, marks)
    return marks


def dense_knot_hash(lengths):
    dense_hash = to_dense_hash(sparse_hash(lengths))
    return ''.join([format(number, 'x').zfill(2) for number in dense_hash])
...
if __name__ == '__main__':
    ...
    with open('input') as f:
        ...
        (marks, _, _) = knot_hash(lengths)
        print(marks[0] * marks[1])

    # part2
    with open('input') as f:
        line = f.readline().strip()
        lengths_bin = list(map(ord, line))
        print(dense_knot_hash(lengths_bin))
